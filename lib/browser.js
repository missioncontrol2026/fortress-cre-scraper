...
async function getContext(vendor) {
  ...
  // Clean stale Chromium singleton locks from crashed processes
...
// newPage() acquires the per-vendor Chromium lock, launches a fresh persistent
// context on the shared userDataDir, and returns a page. When the page closes,
// the context is torn down AND the vendor lock is released so the next scrape
// can start. This guarantees a single Chromium process per vendor at a time.
