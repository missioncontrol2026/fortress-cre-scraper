...
async function getContext(vendor) {
  ...
  if (PROXY_HOST && PROXY_USERNAME && PROXY_PASSWORD) {
    // Chromium can't authenticate to some proxies via the Playwright proxy option
    // (returns ERR_PROXY_AUTH_UNSUPPORTED). Solution: spin up a local proxy-chain
    // relay that forwards to IPRoyal with the auth pre-applied. Chromium then talks
    // to localhost with no auth needed.
    const stickyUser = stickyUsername(PROXY_USERNAME, vendor);
    const upstream = `http://${encodeURIComponent(stickyUser)}:${encodeURIComponent(PROXY_PASSWORD)}@${PROXY_HOST}`;
